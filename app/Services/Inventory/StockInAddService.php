...
class StockInAddService
{
 ...
 public function insertIncomingStockByManual(Request $request)
 {
  ...
  try {
   ...
   $this->insertIncomingBloodLog($incomingBlood, $request, $incomingBloodDetails, $user);
 ...
 private function resolveOrderStatus(int $insertedCount, int $totalQuantity): OrderBloodStatus
 {
  return $insertedCount >= $totalQuantity
 ...
 private function insertOrderLog(OrderBlood $orderBlood, OrderBloodStatus $status, ?User $user): void
 {
  ...
  OrderLogActivity::create([
   ...
   'ordered_at' => $orderBlood->created_at,
  ]);
 }
