...
export function initPrintPreview() {
    ...
    function setLoading(btn, isLoading) {
        ...
        function buildPreviewUrl(btn) {
            ...
            // 1. Ambil data-id dari tombol
            const dataId = btn.dataset.id;
            // Tambahkan query param jika id ada, contoh: ?id=5
            const queryString = dataId ? `?id=${encodeURIComponent(dataId)}` : '';

            const template = btn.dataset.previewUrl;

            if (template) {
                return template.replace(":print", encodeURIComponent(printSlug)) + queryString;
            }

            // 2. Tempelkan queryString di akhir URL default
            return `/blood-transfusion/print/${encodeURIComponent(printSlug)}${queryString}`;
        }
