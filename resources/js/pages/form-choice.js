    ...
    if (elements && elements.length > 0) {
        elements.forEach((item) => {
            const config = {
                ...
                removeItemButton:
                    item.hasAttribute("data-choices-removeItem") ||
                    item.hasAttribute("data-choices-multiple-remove"),
                shouldSort: !item.hasAttribute("data-choices-sorting-false"),
                maxItemCount:
                    item.getAttribute("data-choices-limit") || undefined,
                duplicateItemsAllowed: !item.hasAttribute(
            ...
            if (item.hasAttribute("data-choices-text-disabled-true"))
                instance.disable();
        });
    }
});
