...
$totalCols = 2 + count($components) * count($bloodTypes) + 1;
...
<table>
 <thead>
  ...
  <tr>
   ...
   @foreach ($components as $component)
    <th colspan="{{ count($bloodTypes) }}" style="{{ $headerStyle }}">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td style="border:1px solid #000; white-space:nowrap;">{{ $room['room_name'] }}</td>
   @foreach ($components as $comp)
   @foreach ($bloodTypes as $group)
   <td style="{{ $cellStyle }}">
    {{ $room[$comp['id']][$group] ?: 0 }}
   </td>
   @endforeach
   @endforeach
